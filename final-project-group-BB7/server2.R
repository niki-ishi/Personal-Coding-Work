...
#chart2_data <- chart2_data %>% filter(Group == "By Age") %>% filter(Year == Year) 
...
server <- function(input, output) {
  ...
  output$agePlot <- renderPlotly({
    ...
    chart2_data <- chart2_data %>%  filter(Year %in% input$user_category1) 


    age_plot <- ggplot(data = chart2_data) +
      geom_line(mapping = aes(x = Subgroup, y = avg, group = Year, color = Year)) +
      labs(
        title = " Mental Health by Age During COVID-19", x = "Ages", y = "Avg % of Age Group With Depressive Symptoms")
